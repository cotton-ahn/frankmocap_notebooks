...
def integration_copy_paste(pred_body_list, pred_hand_list, smplx_model, image_shape):
    ...
    for i in range(len(pred_body_list)):
        ...
        pred_joints_vis_img = convert_bbox_to_oriIm(
            pred_joints_vis_bbox, bbox_scale_ratio, bbox_top_left, image_shape[1], image_shape[0])
        
        integral_output['pred_joints_img'] = pred_joints_vis_img
        
        pred_joints_vis = smplx_output.left_hand_joints[0].detach().cpu().numpy()    
        pred_joints_vis_bbox = convert_smpl_to_bbox(pred_joints_vis, camScale, camTrans) 
        pred_joints_vis_img = convert_bbox_to_oriIm(
            pred_joints_vis_bbox, bbox_scale_ratio, bbox_top_left, image_shape[1], image_shape[0])
        
        integral_output['left_hand_joints_img_coord'] = pred_joints_vis_img

        pred_joints_vis = smplx_output.right_hand_joints[0].detach().cpu().numpy()    
        pred_joints_vis_bbox = convert_smpl_to_bbox(pred_joints_vis, camScale, camTrans) 
        pred_joints_vis_img = convert_bbox_to_oriIm(
            pred_joints_vis_bbox, bbox_scale_ratio, bbox_top_left, image_shape[1], image_shape[0])
        
        integral_output['right_hand_joints_img_coord'] = pred_joints_vis_img
